...
import {
  Derivacion,
  ResultDerivacionInternas,
} from 'src/app/interface/derivacion.interna';
import { Respuesta, ResultRespuestas } from 'src/app/interface/respuesta';
import {
  ResultSeguimientos,
  SeguimientoInterno,
} from 'src/app/interface/seguimiento';
  ...
  responderTramite() {
    ...
    if (this.tipoRespuesta === '1') {
      if (
        ...
        this.formRespuesta.get('destino')?.value.length >= 1
      ) {
      ...
      if (this.idDerivado !== 0) {
        ...
        this.respuestaService
          .putRespuestaInternoSinDerivado(formData)
          .subscribe(
            (data) => {
              console.log(data);
              this.mostrarDerivacion();
      ...
      if (
        ...
        this.respuestaService.putRespuestaInternoDerivadoVarios(formData).subscribe(
          (data)=>{
            this.mostrarDerivacion();
            console.log(data);
            this.idDerivado =0;
